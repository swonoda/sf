    ...
    def put_worksdata(self, data):
        ...
        if(len(result) == 0):
            cur.execute("insert into works \
                        ...
                        (data.ordinal, data.name, data.summary_title, data.summary_count, data.body_title, data.body_total_count, data.body_newline, data.body_scat, data.body_ruby, data.score, data.work_url))
            con.commit()
        else:
            print(result)
            print(data.work_url)
            ...
            cur.execute("update works set \
                        ...
                        (data.body_title, data.body_total_count, data.body_newline, data.body_scat, data.body_ruby, data.work_url, data.summary_title))
            con.commit()
        cur.close()
        con.close()
