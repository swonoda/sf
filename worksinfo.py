...
class WorksInfo:
    ...
    def students_scraping(self, student_list):
        for student in student_list:
            ...
            self.work_url = student.find("a", attrs={"class", "anchor-block"}).get("href")
            print(self.work_url)
            link = self.work_url
            self.works_scraping(link)
            self.pg_ctrl.put_worksdata(self.ordinal, self.name, self.summary_title, self.summary_count,
                                       self.work_title, self.newline_count, self.scat_count, self.ruby_count, self.body_count, self.score, self.work_url)
#            self.works_list.append(raw)
    ...
    def update(self) :
        ...
        for update_work in result :
            self.ordinal = update_work[0]
            self.author = update_work[1]
            self.summary_title = update_work[2]
            self.summary_count = update_work[3]
            self.work_title = update_work[4]
            self.work_url = update_work[5]
            self.works_scraping(self.work_url)
            self.pg_ctrl.put_worksdata(self.ordinal, self.name, self.summary_title, self.summary_count,
                                       self.work_title, self.newline_count, self.ruby_count, self.scat_count, self.body_count, self.score, self.work_url)
